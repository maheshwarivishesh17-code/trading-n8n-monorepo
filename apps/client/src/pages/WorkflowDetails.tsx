  ...
  if (loading) {
    return (
      <div className="w-screen h-screen bg-background text-foreground flex items-center justify-center">
        <div className="text-foreground/60">Loading workflow details...</div>
      </div>
    );
  }
    ...
    return (
      <div className="w-screen h-screen bg-background text-foreground flex flex-col">
        <header className="flex items-center px-6 py-4 border-b bg-gradient-to-r from-primary/10 via-transparent to-secondary/5">
          <Link to="/dashboard" className="text-sm text-foreground/80 mr-2">← Back to Dashboard</Link>
        </header>
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <div className="text-4xl mb-4">❌</div>
            <h2 className="text-xl font-medium text-foreground mb-2">Workflow Not Found</h2>
            <Link to="/dashboard">
    ...
    <div className="w-screen h-screen bg-background text-foreground flex flex-col">
      <header className="flex items-center justify-between px-6 py-4 border-b bg-gradient-to-r from-primary/10 via-transparent to-secondary/5">
        <div className="flex items-center gap-4">
          <Link to="/dashboard" className="text-sm text-foreground/80">← Back</Link>
          <div>
            <h1 className="text-xl font-semibold text-foreground">{workflow.name}</h1>
            <p className="text-xs text-foreground/60">ID: {workflow.id}</p>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-auto p-6">
        <div className="max-w-4xl">
          {/* Status Card */}
          <div className="bg-card backdrop-blur border border-border rounded-lg p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-card backdrop-blur border border-border rounded-lg p-4">
              <div className="text-xs text-foreground/60 mb-2">Nodes</div>
              <div className="text-2xl font-bold text-foreground">{workflow.nodes}</div>
            </div>
            <div className="bg-card backdrop-blur border border-border rounded-lg p-4">
              <div className="text-xs text-foreground/60 mb-2">Edges</div>
              <div className="text-2xl font-bold text-foreground">{workflow.edges}</div>
            </div>
            <div className="bg-card backdrop-blur border border-border rounded-lg p-4">
              <div className="text-xs text-foreground/60 mb-2">Executions</div>
              <div className="text-2xl font-bold text-foreground">{workflow.executions}</div>
            </div>
            <div className="bg-card backdrop-blur border border-border rounded-lg p-4">
              <div className="text-xs text-foreground/60 mb-2">Success Rate</div>
          ...
          <div className="bg-card backdrop-blur border border-border rounded-lg p-6">
            <h2 className="text-lg font-semibold text-foreground mb-4">Workflow Details</h2>
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between border-b border-border pb-3">
                ...
                <span className="text-foreground">{workflow.updatedAt}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-foreground/60">Total Executions</span>
                <span className="text-foreground">{workflow.executions}</span>
              </div>
            </div>
          </div>
        </div>
